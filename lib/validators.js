...
function validateExpiration({command}, responseOnFail, responseOnPass) {
  ...
  const valid = record && Date.now() <= record.expiration
  if(valid) return responseOnPass || { record }
  if(record) map.delete(key) // flush expired key
...
function validateFormat(command, format) {
  ...
  for(let i = 1; i < format.length; i++) {
    ...
    if(invalid) return 'CLIENT_ERROR bad command line format'
  }
  return 'OK'
}
