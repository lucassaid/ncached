...
test('Command with wrong format', () => {
  expect(process(`set foo 0 9 A`)).toBe(BAD_COMMAND_FORMAT)
})
...
test('Set command with bad data chunk', () => {
  ...
  expect(process(command)).toBe(BAD_DATA_CHUNK)
})
...
test('Cas command to a non-existing key', () => {
  const casCommand = `cas somekey ${flags} ${exptime} 5 0`
  expect(process(`${casCommand}\r\nredis`)).toMatch(NOT_FOUND)
})
